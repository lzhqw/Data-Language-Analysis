...
def train_one_epoch(net, optimizer, loss, train_loader, device):
    ...
    for batch in t:
        ...
        scores, word_alphas, sentence_alphas = net(documents, sentences_per_document, words_per_sentence)
        l = loss(scores, y)
        optimizer.zero_grad()
        l.backward()
        optimizer.step()
        ...
        true = torch.eq(predict, y).sum().item()
        metrics[1] += true
        metrics[0] += l.item() * y.shape[0]
        metrics[2] += y.shape[0]
...
def train(net, train_loader_params, test_loader, lr, epochs, device, weight_path, num_classes, logpath='', weight=None):
    # -------------------------------------------- #
    # 网络、优化器、loss
    # -------------------------------------------- #
    net.to(device)
    ...
    # loss = MultiFocalLoss(num_class=num_classes, alpha=weight, gamma=2)
...
def visualize_attention(doc, scores, word_alphas, sentence_alphas, words_in_each_sentence, y, category,idx):
    """
    ...
    prediction = '{category} ({score:.2f}%)'.format(category=prediction, score=score * 100)
    ...
    # Display
    img.save('imgmulti/' + f'{prediction[:-8]} {y} {idx} {score:.3f}  ' + ''.join(
